...
The `describe` subcommand MUST accept one positional argument in the form `<db>.<table>` consisting of two identifiers, each matching the regex `^[A-Za-z0-9_-]+$`, joined by exactly one `.` separator. The input MUST split on `.` into exactly two non-empty segments; any other segment count (zero, one, three or more) emits `CONFIG_ERROR` (exit 2) and MUST NOT reach the database. On valid input the subcommand MUST execute `` DESCRIBE `<db>`.`<table>` `` (each identifier backtick-wrapped) and emit a success envelope whose `columns` are `Field`, `Type`, `Null`, `Key`, `Default`, `Extra` and whose `rows` is one entry per column of the table. The subcommand MUST also be invokable as `desc`; `desc` is a strict alias with no behavioural difference.
