...
- **Metadata subcommand argument whitelist**: `tables` and `describe` validate their argument against `[A-Za-z0-9_-]+` (with the dot separator accepted only in `describe`, which additionally requires exactly one dot and exactly two non-empty segments); anything else is `CONFIG_ERROR`. Identifiers are backtick-wrapped when building the SQL.
